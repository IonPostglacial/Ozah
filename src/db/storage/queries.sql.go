...
func (q *Queries) GetDocumentHierarchy(ctx context.Context, path string) ([]Document, error) {
	rows, err := q.db.QueryContext(ctx, getDocumentHierarchy, path)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
left join Document_Translation tr1 on doc.Ref = tr1.Document_Ref and tr1.Lang_Ref = ?1
where (doc.Path >= ?2 and doc.Path < (?2 || '.~'))
order by (Path || '.' || Ref) asc
`
...
func (q *Queries) GetDocumentHierarchyTr1(ctx context.Context, arg GetDocumentHierarchyTr1Params) ([]GetDocumentHierarchyTr1Row, error) {
	rows, err := q.db.QueryContext(ctx, getDocumentHierarchyTr1, arg.Lang1, arg.Path)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
left join Document_Translation tr1 on doc.Ref = tr1.Document_Ref and tr1.Lang_Ref = ?1
left join Document_Translation tr2 on doc.Ref = tr2.Document_Ref and tr2.Lang_Ref = ?2
where (doc.Path >= ?3 and doc.Path < (?3 || '.~'))
order by (Path || '.' || Ref) asc
`
...
func (q *Queries) GetDocumentHierarchyTr2(ctx context.Context, arg GetDocumentHierarchyTr2Params) ([]GetDocumentHierarchyTr2Row, error) {
	rows, err := q.db.QueryContext(ctx, getDocumentHierarchyTr2, arg.Lang1, arg.Lang2, arg.Path)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
